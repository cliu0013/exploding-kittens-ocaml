  ...
  | BOMBED ->
      let game =
        if Deck.player_have_card e.game curr_id "Defuse" then
          if ai_mercy_dice 20 then (
            print_endline
              ("AI player " ^ string_of_int curr_id
             ^ " doesn't want to use the Defuse :)");
            ...
            print_endline
             ...
             ^ " draws a BOMB but it decides to use 'Defuse'! What a \
                genius");
            let temp = Deck.use_card e.game curr_id "Defuse" 1 in
            Deck.place_bomb temp (dice_res 20))
        else (
          print_endline
      ...
      if e.num_alive = 0 then user_win e else turn_start e
  | _ ->
      let game =
        ...
        in
        let temp =
          if name = "Exploding Kitten" then
            Deck.change_state temp e.curr_id BOMBED
          else Deck.change_state temp e.curr_id SAFE
        in
        if debug then
          print_endline
            ("Player " ^ (e.curr_id |> string_of_int) ^ " drew: " ^ name)
